...
def add_indicators(df: pd.DataFrame, cfg: StrategyConfig) -> pd.DataFrame:
    ...
    # Add EMA trend filter if enabled
    if cfg.filters.ema_trend.use:
        logger.debug(f"Computing EMA trend filter (length={cfg.filters.ema_trend.length})")
        ema = df.ta.ema(length=cfg.filters.ema_trend.length)
        result = result.join(ema.rename("EMA_TREND"))
    
    # Add ATR if risk management is enabled
    if cfg.risk.use_atr:
        logger.debug(f"Computing ATR (length={cfg.risk.atr_length})")
        atr = df.ta.atr(length=cfg.risk.atr_length)
        result = result.join(atr.rename("ATR"))
